...
export default function PageBanner({ title, subtitle, breadcrumb, isHome, actions, onMenuToggle }) {
  ...
  return (
    <div className="relative flex-shrink-0">
      {/* Banner image — constrained on mobile, natural on desktop */}
      <img
        src={isHome ? '/banner-home.jpg' : '/banner-page.png'}
        alt=""
        className={`w-full block object-cover ${isHome ? 'h-[130px] sm:h-[150px] lg:h-auto' : 'h-[88px] sm:h-[100px] lg:h-auto'}`}
        draggable={false}
      />
      ...
      {/* User chip — responsive: avatar-only on mobile, full chip on sm+ */}
      {user && (

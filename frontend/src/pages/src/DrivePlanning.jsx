...
import { MOCK_ALERTS, MOCK_HOTSPOTS, MOCK_RECOMMENDED_DRIVE } from './mockData'
import {
...
export default function DrivePlanning() {
  ...
  const alertIdParam = searchParams.get('alertId') || MOCK_ALERTS[0].id
  ...
  const drive = MOCK_RECOMMENDED_DRIVE

  const handlePlanDrive = () => {
  ...
  return (
    ...
    >
            ...
            <MapContainer center={SG_CENTER} zoom={11} style={{ height: '100%', width: '100%' }} scrollWheelZoom={false}>
              ...
              {MOCK_HOTSPOTS.map((h) => (
                <Circle
                  ...
                  center={h.pos}
                  radius={3000}
                  pathOptions={{ color: 'transparent', fillColor: h.color, fillOpacity: 0.25 }}
                />
              ))}
              {MOCK_HOTSPOTS.map((h) => (
                <Marker
      ...
      {/* Modals */}
      {modal === 'why'   && <WhyModal drive={drive} onClose={() => setModal(null)} />}
      {modal === 'score' && <ScoreModal drive={drive} onClose={() => setModal(null)} />}
      {modal === 'alt'   && <AltLocationsModal drive={drive} onClose={() => setModal(null)} />}
    </PageLayout>
  )
}

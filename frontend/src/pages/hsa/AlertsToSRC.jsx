...
const STATUS_TABS = ['All', 'Draft', 'New', 'Sent', 'Past']
...
function getStatusBadge(status) {
  ...
  if (status === 'Past') return 'text-gray-600 bg-gray-100'
            ...
            <div className="flex gap-1 mb-3 flex-wrap">
              {STATUS_TABS.map(t => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className={`relative px-2 py-0.5 rounded-full text-xs font-medium z-0 ${tab === t ? 'text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'
                    }`}
                >
                  {tab === t && (
                    <motion.span
                ...
                {filteredAlerts.length === 0 ? (
                  ...
                  filteredAlerts.map(alert => (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      className={`w-full text-left card p-3 cursor-pointer transition-all hover:border-primary/30 ${selectedId === alert.id ? 'border-primary ring-1 ring-primary/20' : ''
                        }`}
                    >

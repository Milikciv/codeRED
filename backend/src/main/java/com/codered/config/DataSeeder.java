...
public class DataSeeder implements CommandLineRunner {
    ...
    private void ensureSrcDataExists() {
        if (srcAlertRepository.count() == 0)
            seedSrcAlerts();
        if (donationDriveRepository.count() == 0)
            seedDonationDrives();
        if (donorRepository.count() == 0)
            seedDonors();
        if (donorDemographicRepository.count() == 0)
    ...
    /**
     * Idempotent backfill: update existing drives that are still using a single
     * alertCode
     * to their correct multi-alert values so that re-seeded and migrated DBs look
     * the same.
     */
    ...
    private void seedDonors() {
        ...
        String[] regions = { "Tampines", "Jurong East", "Woodlands", "Ang Mo Kio", "Bedok", "Bukit Batok", "Clementi",
                "Yishun" };
        ...
        // Distribution approximates Singapore prevalence: O+(34%), A+(25%), B+(16%),
        // O-(7%), A-(6%), AB+(5.5%), B-(3.2%), AB-(2.7%)
        int[][] batches = { { 0, 82 }, { 1, 61 }, { 2, 38 }, { 3, 17 }, { 4, 14 }, { 5, 13 }, { 6, 8 }, { 7, 7 } };
        BloodType[] bts = BloodType.values();

        // Birth year pool giving target age distribution (16-20: 4%, 21-30: 30%, 31-40:
        // 27%, 41-50: 22%, 51-60: 13%, 60+: 4%)
        ...
        for (int[] batch : batches) {
            ...
            for (int ci = 0; ci < count; ci++) {
                ...
                // Status cycle (12-slot): 5 dormant (42%), 2 active-recent (17%), 5
                // active-eligible (42%)
                int slot = ci % 12;
                if (slot < 5) {
                    // Dormant
    ...
    private void seedRecommendedDrive() {
        record AltEntry(String location, int eligibleDonors, int pastSuccessRate, int confidenceScore, double lat,
                double lng,
        ...
        String[] breakdownCriteria = { "Eligible donor density", "Low recent donation activity", "Accessibility",
                "Nearby amenities", "Past drive success" };
        ...
        List<DriveEntry> entries = List.of(
                new DriveEntry("ALT-2505-001", "O-", "Sat, 31 May 2026", "10:00 AM", "4:00 PM", 86, 18, 72, 87,
                        ...
                        new String[][] {
                                { "High donor density",
                                        "Over 86 eligible donors live within 5km, giving this drive a large pool to draw from." },
                                { "Strong community engagement",
                                        "Tampines has historically shown high participation in community health events." },
                                { "Good MRT connectivity",
                                        "Located near Tampines MRT, making it easily accessible for donors island-wide." },
                                { "High past success rate",
                                        "72% of past drives here met or exceeded their donation targets." },
                        },
                        ...
                        new AltEntry[] {
                                new AltEntry("Woodlands Civic Centre", 74, 65, 78, 1.4382, 103.7891,
                                        ...
                                        new String[][] {
                                                { "Strong foot traffic",
                                                        "The civic centre sees high daily foot traffic from residents and commuters." },
                                                { "North-region coverage",
                                                        "Covers Woodlands and surrounding estates with a large eligible donor base." },
                                                { "Community familiarity",
                                                        "Regular community events at the venue improve donor awareness and participation." },
                                                { "MRT accessibility",
                                                        "Served by Woodlands MRT on the North-South Line." },
                                        },
                                ...
                                new AltEntry("Jurong East CC", 68, 60, 72, 1.3330, 103.7430,
                                        ...
                                        new String[][] {
                                                { "Major transport hub",
                                                        "Jurong East interchange connects multiple MRT lines and numerous bus routes." },
                                                { "West-region donor reach",
                                                        "Covers Jurong East and nearby estates, tapping a sizeable O- eligible pool." },
                                                { "Established venue",
                                                        "CC has hosted multiple past drives with adequate facilities and parking." },
                                                { "Good past success",
                                                        "60% past success rate reflects a consistent local donor response." },
                                        },
                                ...
                                new AltEntry("Bishan CC", 55, 58, 65, 1.3510, 103.8480,
                                        ...
                                        new String[][] {
                                                { "Central island location",
                                                        "Bishan's central position makes it accessible from north, east, and west regions." },
                                                { "Multi-line MRT access",
                                                        "Served by both the North-South and Circle Lines at Bishan MRT." },
                                                { "Active CC programmes",
                                                        "Regular health and community events keep residents familiar with donation drives." },
                                                { "Moderate donor base",
                                                        "55 eligible donors provides a workable target pool for O- outreach." },
                                        },
                ...
                new DriveEntry("ALT-2505-002", "B+", "Fri, 7 Jun 2026", "9:00 AM", "3:00 PM", 75, 22, 68, 82,
                        ...
                        new String[][] {
                                { "Central location",
                                        "Jurong East is a major transport hub, accessible from multiple MRT lines and bus routes." },
                                { "Large venue capacity",
                                        "The sports centre can accommodate high donor volumes with minimal wait times." },
                                { "High-response donor base",
                                        "22 high-response donors in the area are likely to turn up reliably." },
                                { "Consistent past performance",
                                        "68% past success rate indicates a reliable and motivated local donor community." },
                        },
                        ...
                        new AltEntry[] {
                                new AltEntry("Tampines Hub", 62, 63, 74, 1.3527, 103.9453,
                                        ...
                                        new String[][] {
                                                { "High visitorship",
                                                        "Tampines Hub integrates a library, sports centre, and hawker centre, drawing large crowds." },
                                                { "East-region coverage",
                                                        "Covers Tampines and Pasir Ris with a healthy B+ eligible donor pool." },
                                                { "Good past performance",
                                                        "63% past success rate reflects reliable turnout from east-region donors." },
                                                { "Strong transport links",
                                                        "Well-connected via Tampines MRT and major bus routes." },
                                        },
                                ...
                                new AltEntry("Yishun CC", 58, 60, 70, 1.4295, 103.8353,
                                        ...
                                        new String[][] {
                                                { "Large residential catchment",
                                                        "Yishun is one of Singapore's largest towns with a high donor-eligible population." },
                                                { "North-region access",
                                                        "Covers Yishun and nearby Sembawang, expanding outreach coverage." },
                                                { "Active CC network",
                                                        "Yishun CC regularly organises community events, improving drive visibility." },
                                                { "MRT and bus access",
                                                        "Served by Yishun MRT and multiple feeder bus routes." },
                                        },
                                ...
                                new AltEntry("Toa Payoh Hub", 50, 55, 63, 1.3318, 103.8470,
                                        ...
                                        new String[][] {
                                                { "Central location",
                                                        "Toa Payoh Hub is easily accessible from multiple parts of Singapore." },
                                                { "Established community venue",
                                                        "A long-standing community destination with strong resident familiarity." },
                                                { "MRT proximity",
                                                        "Located near Toa Payoh MRT on the North-South Line." },
                                                { "Reliable past participation",
                                                        "55% past success rate indicates consistent donor engagement in the area." },
                                        },
                ...
                new DriveEntry("ALT-2505-003", "A-", "Sat, 14 Jun 2026", "10:00 AM", "5:00 PM", 68, 15, 75, 79,
                        ...
                        new String[][] {
                                { "Proven track record",
                                        "75% past success rate is among the highest across all recommended locations." },
                                { "Strong local engagement",
                                        "Woodlands CC runs active community programmes that drive foot traffic and awareness." },
                                { "Excellent public transport access",
                                        "Well-served by Woodlands MRT and multiple bus routes for easy donor access." },
                                { "Eligible donor pool",
                                        "68 eligible donors within 5km ensures sufficient reach for the A- blood type target." },
                        },
                        ...
                        new AltEntry[] {
                                new AltEntry("Bedok Community Centre", 60, 68, 73, 1.3236, 103.9273,
                                        ...
                                        new String[][] {
                                                { "High past success rate",
                                                        "68% past success rate is the highest among this drive's alternatives." },
                                                { "East-region donor coverage",
                                                        "Covers Bedok and surrounding estates with a solid A- eligible pool." },
                                                { "Community drive experience",
                                                        "Bedok CC has hosted multiple drives with experienced logistics and setup." },
                                                { "Transport accessibility",
                                                        "Served by Bedok MRT and a major bus interchange." },
                                        },
                                ...
                                new AltEntry("Clementi CC", 55, 62, 68, 1.3154, 103.7649,
                                        ...
                                        new String[][] {
                                                { "University-adjacent location",
                                                        "Proximity to NUS broadens the eligible donor demographic to include younger adults." },
                                                { "Dense residential area",
                                                        "Clementi's high-density housing ensures a sizeable local donor pool." },
                                                { "Good past performance",
                                                        "62% past success rate reflects reliable donor participation in the west." },
                                ...
                                new AltEntry("Ang Mo Kio CC", 48, 58, 61, 1.3691, 103.8454,
                                        ...
                                        new String[][] {
                                                { "Mature estate engagement",
                                                        "AMK residents have strong familiarity with community health programmes." },
                                                { "Central-north coverage",
                                                        "Offers complementary geographic reach to supplement north-region drives." },
                                                { "Active civic participation",
                                                        "Regular CC events build awareness and drive footfall for donation campaigns." },
                                                { "MRT and bus access",
                                                        "Served by Ang Mo Kio MRT and extensive feeder bus routes." },
                                        },
                ...
                new DriveEntry("ALT-2505-004", "O+", "Sun, 22 Jun 2026", "10:00 AM", "4:00 PM", 92, 25, 70, 85,
                        ...
                        new String[][] {
                                { "Largest donor catchment",
                                        "92 eligible donors nearby — the highest of all recommended locations." },
                                { "Top high-response donors",
                                        "25 high-response donors make Bedok the strongest location for reliable turnout." },
                                { "Strong past drive performance",
                                        "Previous drives at Bedok CC consistently attracted strong donor numbers." },
                                { "Excellent accessibility",
                                        "Served by Bedok MRT and major bus interchanges, convenient for the wider east region." },
                        },
                        ...
                        new AltEntry[] {
                                new AltEntry("Jurong West CC", 80, 67, 79, 1.3479, 103.7069,
                                        ...
                                        new String[][] {
                                                { "Large western donor pool",
                                                        "80 eligible donors in the vicinity — the highest among this drive's alternatives." },
                                                { "Strong past success rate",
                                                        "67% past success rate reflects consistently high turnout at this venue." },
                                                { "Large estate coverage",
                                                        "Jurong West is one of Singapore's most populous towns, maximising outreach potential." },
                                                { "Good transport connectivity",
                                                        "Accessible via Boon Lay MRT and multiple major bus routes." },
                                        },
                                ...
                                new AltEntry("Sembawang CC", 72, 63, 74, 1.4491, 103.8185,
                                        ...
                                        new String[][] {
                                                { "North-region coverage",
                                                        "Covers Sembawang and Canberra estates, reaching donors not served by east-region drives." },
                                                { "Diverse donor demographic",
                                                        "Proximity to HDB estates and military housing expands the eligible donor profile." },
                                                { "Active CC engagement",
                                                        "Sembawang CC runs regular health outreach programmes that support drive visibility." },
                                                { "Bus and MRT access",
                                                        "Served by Sembawang MRT and feeder bus routes." },
                                        },
                                ...
                                new AltEntry("Queenstown CC", 65, 60, 68, 1.2944, 103.8060,
                                        ...
                                        new String[][] {
                                                { "Central-south positioning",
                                                        "Provides geographic coverage in the south, complementing east and north drives." },
                                                { "MRT accessibility",
                                                        "Close to Queenstown MRT on the East-West Line for easy donor access." },
                                                { "Mature estate participation",
                                                        "Queenstown is one of Singapore's oldest towns with strong community health awareness." },
                                                { "Steady donor engagement",
                                                        "60% past success rate reflects consistent outreach outcomes in this area." },
                                        },
        ...
        for (DriveEntry e : entries) {
            ...
            Optional<RecommendedDrive> existingTop = recommendedDriveRepository.findByAlertCodeAndRank(e.alertCode(),
                    1);
            if (existingTop.isEmpty() || existingTop.get().getReasons().isEmpty()) {
                RecommendedDrive rd = existingTop.orElse(new RecommendedDrive());
                rd.setAlertCode(e.alertCode());
                rd.setRank(1);
                rd.setLocation(e.location());
                rd.setBloodType(e.bloodType());
                rd.setDate(e.date());
                rd.setStartTime(e.startTime());
                rd.setEndTime(e.endTime());
                rd.setEligibleDonors(e.eligibleDonors());
                rd.setHighResponseDonors(e.highResponseDonors());
                rd.setPastSuccessRate(e.pastSuccessRate());
                rd.setConfidenceScore(e.confidenceScore());
                rd.setLatitude(e.lat());
                rd.setLongitude(e.lng());

                rd.getReasons().clear();
                RecommendedDriveReason narrativeReason = new RecommendedDriveReason();
                narrativeReason.setLabel("Narrative");
                narrativeReason.setDetail(e.narrative());
                ...
                for (int i = 0; i < bd.length; i++) {
                    ...
                    s.setCriterion(breakdownCriteria[i]);
                    s.setWeight(bd[i][0]);
                    s.setScore(bd[i][1]);
            ...
            for (AltEntry alt : e.alternatives()) {
                int altRank = rank++;
                Optional<RecommendedDrive> existingAlt = recommendedDriveRepository
                        .findByAlertCodeAndRank(e.alertCode(), altRank);
                if (existingAlt.isPresent() && !existingAlt.get().getReasons().isEmpty())
                    continue;

                RecommendedDrive altDrive = existingAlt.orElse(new RecommendedDrive());
                altDrive.setAlertCode(e.alertCode());
                altDrive.setRank(altRank);
                altDrive.setLocation(alt.location());
                altDrive.setBloodType(e.bloodType());
                altDrive.setDate(e.date());
                altDrive.setStartTime(e.startTime());
                altDrive.setEndTime(e.endTime());
                ...
                altDrive.setLatitude(alt.lat());
                altDrive.setLongitude(alt.lng());

                altDrive.getReasons().clear();
                RecommendedDriveReason altNarrative = new RecommendedDriveReason();
                altNarrative.setLabel("Narrative");
                altNarrative.setDetail(alt.narrative());
                ...
                for (int i = 0; i < alt.breakdown().length; i++) {
                    RecommendedDriveScoreBreakdown s = new RecommendedDriveScoreBreakdown();
                    s.setRecommendedDrive(altDrive);
                    s.setCriterion(breakdownCriteria[i]);
                    s.setWeight(alt.breakdown()[i][0]);
                    s.setScore(alt.breakdown()[i][1]);
    ...
    private void seedCollaborators() {
        ...
        Object[][] data = {
                { "DBS Bank", "csr@dbs.com", "Tampines Central 1", 1.3547, 103.9442, "4,500 employees", 82, "Companies",
                        new String[] { "Large Company", "Health Partner" } },
                { "Singapore Airlines", "community@singaporeair.com", "Airline House, 25 Airline Rd", 1.3357, 103.8594,
                        "3,200 employees", 76, "Companies", new String[] { "Large Company", "Health Partner" } },
                { "CapitaLand", "csr@capitaland.com", "168 Robinson Rd", 1.2791, 103.8490, "2,800 employees", 61,
                        "Companies", new String[] { "Large Company", "Community Partner" } },
                { "ST Engineering", "community@stengg.com", "1 Ang Mo Kio Electronics Park", 1.3695, 103.8591,
                        "2,100 employees", 70, "Companies", new String[] { "Large Company" } },
                { "Temasek Polytechnic", "studentaffairs@tp.edu.sg", "21 Tampines Ave 1", 1.3456, 103.9311,
                        "2,000 students", 92, "Schools", new String[] { "Polytechnic", "Youth Partner" } },
                { "SUTD", "studentlife@sutd.edu.sg", "8 Somapah Rd", 1.3414, 103.9635, "1,800 students", 85, "Schools",
                        new String[] { "University", "Research Partner" } },
                { "ITE College East", "studentservices@ite.edu.sg", "10 Simei Ave", 1.3428, 103.9424, "1,500 students",
                        78, "Schools", new String[] { "ITE", "Youth Partner" } },
                { "Temasek JC", "general@temasekjc.moe.edu.sg", "22 Bedok South Rd", 1.3254, 103.9324, "1,200 students",
                        81, "Schools", new String[] { "Junior College", "Youth Partner" } },
                { "Community Clubs", "pa_tampines@pa.gov.sg", "Tampines Town Council", 1.3521, 103.9406,
                        "3,000 members", 90, "Community Groups", new String[] { "Community", "Grassroots" } },
                { "Religious Organisations", "outreach@ircc.sg", "Various Locations", 1.3530, 103.9420, "1,500 members",
                        86, "Community Groups", new String[] { "Faith-based", "Community" } },
                { "Grassroots Organisations", "tampines@grassroots.gov.sg", "Tampines GRC", 1.3510, 103.9380,
                        "2,200 members", 88, "Community Groups", new String[] { "Grassroots", "Community" } },
        };

        for (Object[] row : data) {
            if (collaboratorRepository.existsByName((String) row[0]))
                continue;
    ...
    private void ensureAdminExists() {
        if (userRepository.findByEmail("admin@codered.sg").isPresent())
            return;
        User admin = new User();
    ...
    private void ensureSrcExists() {
        if (userRepository.findByEmail("winnie@redcross.org.sg").isPresent())
            return;
    ...
    private void ensureMultiTypeRequestsExist() {
        ...
        bloodRequestRepository.findByRequestId("REQ-3330").ifPresentOrElse(r -> {
            if (r.getBloodItems().isEmpty()) {
                addBloodItems(r, new BloodType[] { BloodType.O_POSITIVE, BloodType.B_POSITIVE, BloodType.A_NEGATIVE },
                        new int[] { 20, 10, 5 });
    ...
    private void ensureHsaExists() {
        if (hospitalRepository.findByCode("HSA").isPresent())
            return;
    ...
    private void seedHospitals() {
        ...
        for (String[] h : hospitals) {
            if (hospitalRepository.findByCode(h[0]).isPresent())
                continue;
    ...
    private void seedAlerts() {
        ...
        Object[][] data = {
                ...
                { "ALT-2605-004", "AB-", Priority.MEDIUM, "New",
